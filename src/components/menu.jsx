...
export default function Drinks(props) {
  ...
  function scrollCategoryButtons() {
    ...
    if (CATEGORY_BTNS_DIV.current) {
      setTimeout(() => {
        ...
        CATEGORY_BTNS_DIV?.current.childNodes[
          Object.keys(props.menuItems).length - 1
  ...
  function handleFilterChange(category, d) {
    clearTimeout(TIMEOUT);
    CATEGORY_BTNS_DIV?.current.childNodes[d].scrollIntoView({
      block: 'nearest',
      behavior: 'smooth',
    });
    setFilter(category);
  ...
  return (
    <>
      <div
        ...
        {Object.keys(props.menuItems).map((category, d) => {
          return (
            <button
              key={d + 1}
              className={filter == category ? styles.active : null}
              onClick={() => {
                handleFilterChange(category, d + 1);
              }}>
              {category}
            </button>
          );
      ...
      <h1 className='centerHeading'>
        Creekside <span>{props.dataCategory}</span> menu
      </h1>
